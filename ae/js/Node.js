	...
	static create(tag, attributes, content)
	{
		...
		if( content )
		{
			...
			else
			{
				...
				for( let i = 0; i < content.length; i++ ) if( Array.isArray(content[i]) ) content.splice.apply(content, [i,1].concat(content[i]));
				node.append.apply(node, content.filter(c => c !== null));
			}
		}
		...
		if( attributes )
		{
			Object.keys(attributes).forEach((key) =>
			{
				if( typeof attributes[key] === 'function' )
					...
					Object.keys(attributes.style).forEach((k) => { node.style[k] = attributes.style[k]; });
				else
